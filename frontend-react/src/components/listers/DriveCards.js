...
class DriveCards extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            open:false,
    ...
    async componentDidMount () {
        ...
        let temp = await axios.get(axios.fixUrl('/drives'));
        this.setState({
            ...this.state,
            values: temp.data._embedded.drives,
            newValue: {
                'fileName': '',
                'fileType': '',
                'userId': 0,
            ...
            <div>
                ...
                >
                    ...
                    >
                        <Grid container>
                            <Grid item xs>
                                {<Drive offline={this.state.offline} isNew={true} editMode={this.state.editMode} value={this.state.newValue} onChange={this.handleChange}
                        ...
                        {this.state.values.map((value,index) => {
                            return (
                                <Grid item xs={4} key={index}>
                                    <Drive offline={this.state.offline} keyIndex={index} onChange={this.handleListChange.bind(this, index)} editMode={value.editMode}
