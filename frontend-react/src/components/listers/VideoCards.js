...
class VideoCards extends React.Component {
    ...
    async componentDidMount () {
        ...
        let temp = await axios.get(axios.fixUrl('/videos'));
        this.setState({
            ...this.state,
            values: temp.data._embedded.videos,
            newValue: {
                'fileId': 0,
                'videoUrl': '',
                'userId': '',
                'processedDate': '2023-03-22',
            }
        });
    }
            ...
            <div>
                ...
                >
                    ...
                    >
                        <Grid container>
                            <Grid item xs>
                                {<Video offline={this.state.offline} isNew={true} editMode={this.state.editMode} value={this.state.newValue} onChange={this.handleChange}
                        ...
                        {this.state.values.map((value,index) => {
                            return (
                                <Grid item xs={4} key={index}>
                                    <Video offline={this.state.offline} keyIndex={index} onChange={this.handleListChange.bind(this, index)} editMode={value.editMode}
